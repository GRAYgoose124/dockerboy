...
def save_config(config, cfg_file):
    ...
    if os.path.exists(cfg_file):
        ...
        print(
            f"Config file {cfg_file} already exists, backing up to {cfg_file}.bak")
...
def interactive_config_builder(cwd=False):
    ...
    config["interactive"] = False if input(
        "Interactive mode? (Y/n): ").lowercase() != "y" else True
    config["post_removal"] = False if input(
        "Post removal? (Y/n): ").lowercase() != "y" else True
    config["rebuild"] = False if input(
        "Rebuild? (Y/n): ").lowercase() != "y" else True
    return config
